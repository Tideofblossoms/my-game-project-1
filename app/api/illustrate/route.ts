...
export async function POST(request: Request) {
  ...
  if (!apiKey) return NextResponse.json({ error: "OpenAI API is not connected", image: null, live: false }, { status: 503 });
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: failure?.error?.message || "AI image generation failed", image: null, live: false }, { status: response.status });
    }
    ...
    return NextResponse.json({ error: error instanceof Error && error.name === "AbortError" ? "AI image generation timed out" : "AI image service is temporarily unavailable", image: null, live: false }, { status: 502 });
  } finally {
